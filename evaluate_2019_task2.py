...
def main(ref_file, out_file):
    ...
    reference_dict = readFile(ref_file)
    output_dict = readFile(out_file)
    _, _, acc, f1, prec, recall = manipulate_data(input_pairs(reference, output))
    return acc, f1, prec, recall, reference_dict, output_dict
    #print(*["{0:.2f}".format(v) for v in results], sep='\t')

if __name__ == "__main__":
    args = parse_args()
    logging.basicConfig(level=args.verbose)
    acc, f1, prec, recall, _,_ = main(args.reference, args.output)
