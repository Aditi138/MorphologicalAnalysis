...
```
A set of files with POS in the 4th coloumn and remaining tags in the 6th coloumn.
...
```
       Additionally, it will create the following files for training an independent POS tagger:
       ```
       POS_train.conllu
       POS_dev.conllu
       POS_test.conllu ```
       which are in the conll format:
       ```होते	V ```
...
``` python main.py \
    ...
    Include language codes which are used during training the model in ````--langs ```. If you want a zero-shot transfer, you need to include those target languages in ```--augVocablang```.
